<template>
  <div class="container mx-auto flex justify-center relative">
    <div class="max-w-sm sm:max-w-lg w-full space-y-2">
      <h3 class="text-sm sm:text-base font-semibold text-center">База данных</h3>
    ...
    <div v-if="rows.length > 0" class="absolute bottom-4 left-0 right-0 flex justify-center" style="z-index: 1000;">
      <Tooltip :text="selectedRows.length ? 'Удалить запись' : 'Добавить запись'">
        <button
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition duration-300" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-300" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showDeleteConfirmModal" @wheel.prevent @touchmove.prevent @scroll.prevent @click="showDeleteConfirmModal = false" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-[9999]">
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition duration-300" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-300" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showErrorModal" @wheel.prevent @touchmove.prevent @scroll.prevent @click="showErrorModal = false" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-[9999]">
