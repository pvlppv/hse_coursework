            ...
            <form @submit.prevent="save" class="space-y-4">
              <div class="space-y-1">
                <label @click.stop.prevent class="text-sm sm:text-base font-medium flex items-center gap-2">
                  ...
                  <button 
                    type="button"
                    v-tippy="'Название сеанса может отображать, например, общую тему событий, которые ты хочешь отследить, или вопросы, на которые хочешь ответить'"
                    @click.stop
              ...
              <div class="space-y-1">
                ...
                <div v-if="editableSession.goalType === 'experiment' && editableSession.metric" class="space-y-3 mt-4 p-3 border rounded-lg bg-gray-50">
                  ...
                  <div v-if="editableSession.metric.type === 'count'">
                    <input type="text" v-model="editableSession.metric.filterValue" placeholder="Определённого значения (необязательно)" class="w-full p-2 text-xs sm:text-sm border rounded-lg focus:border-gray-300 focus:ring-0"/>
                  </div>
                  <p v-if="editableSession.metric.type === 'average' || editableSession.metric.type === 'sum'" class="text-xs text-gray-500">
          ...
          <div class="mt-5 mb-2 flex-shrink-0">
            <div class="flex justify-center gap-3">
              ...
              <button
                type="submit"
                @click="save"
                :disabled="!editableSession.title || !editableSession.end_time || editableSession.visualization_preferences.length === 0 || !editableSession.goalType"
...
watch(() => props.session, (newSession) => {
  if (newSession) {
    editableSession.value = {
      ...
      goalType: newSession.goal_type || null,
      metric: newSession.metric ? { ...newSession.metric } : null
    };

    if (editableSession.value.goalType === 'experiment') {
      editableSession.value.metric = {
        type: newSession.metric.type || 'count',
        operator: newSession.metric.operator || '>=',
        targetValue: newSession.metric.targetValue || 1,
        filterValue: newSession.metric.filterValue || ''
      };
    }
  } else {
    editableSession.value = null;
    metricBackup.value = null;
  }
...
watch(() => editableSession.value?.goalType, (newGoalType) => {
  ...
  if (newGoalType === 'experiment') {
    if (editableSession.value.metric === null) {
      editableSession.value.metric = metricBackup.value || {
        type: 'count',
        operator: '>=',
        targetValue: 1,
        filterValue: ''
    ...
    if (editableSession.value.metric !== null) {
      metricBackup.value = JSON.parse(JSON.stringify(editableSession.value.metric));
  ...
  if (newGoalType) {
    ...
    if (editableSession.value.visualization_preferences) {
      editableSession.value.visualization_preferences = editableSession.value.visualization_preferences.filter(
...
const filteredVisualizations = computed(() => {
  if (!editableSession.value || !editableSession.value.goalType) {
    return props.visualizations;
  }
  return props.visualizations.filter(v => v.goalType.includes(editableSession.value.goalType));
});
...
const confirmVizSelection = () => {
  if (selectedViz.value) {
    ...
    if (!editableSession.value.visualization_preferences.includes(vizId)) {
        editableSession.value.visualization_preferences.push(vizId);
    }
  }
...
const save = () => {
  emit('save', { ...editableSession.value });
};
