    ...
    <div class="container mx-auto flex justify-center">
        <div class="max-w-sm sm:max-w-lg w-full space-y-5">
            ...
            <div v-else class="px-6 py-4 sm:px-7 sm:py-5 border border-gray-300 rounded-lg">
                <div v-if="projects.length === 0" class="text-center text-xs sm:text-sm text-gray-500">
                    {{ `${timeRanges[selectedRange]} нет проектов` }}
                </div>
                <div v-else class="max-h-60 overflow-y-auto space-y-2.5">
                    <div v-for="project in projects" :key="project.id" 
                        class="flex justify-between items-center">
                        <span class="text-xs sm:text-sm font-semibold">{{ project.name }}</span>
                        <div class="flex space-x-0.5 sm:space-x-1">
                            <div
                                ...
                                :class="getColor(n, project.score)"
                            >
...
// Reactive state for projects and error
const projects = ref([]);
const error = ref(null);

// Function to fetch data based on the selected range
const fetchProjects = async () => {
    const { data, error: fetchError, fetchData } = useFetch(`${apiBaseUrl}/api/project_score/${selectedRange.value}`);
...
watch(selectedRange, fetchProjects);
